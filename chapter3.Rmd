...
odds_ratio <- exp(coef(alc_glm_first)[c(1,2,3,6,10,12)])
odds_df <- data.frame(tibble::rownames_to_column(as.data.frame(odds_ratio)))
odds_df$rowname <- tolower(odds_df$rowname)
odds_df[odds_df$rowname == "absnt>8",]$rowname <- "abseentism"
names(odds_df) <- c("predictor","odds_ratio")
odds_df$odds_ratio <- round(odds_df$odds_ratio, 2)
model_summary_table <- data.frame(model_summary$table[c(1,2,3,6,10,12),])
model_summary_table[model_summary_table$predictor == "Absnt>8",]$predictor <- "abseentism"
...
```{r odds_ratio_final, echo = FALSE, message = FALSE, eval = TRUE}
model_summary <- apa_print(alc_glm_second)
odds_ratio <- exp(coef(alc_glm_second)[c(1,2,3,6,10)])
odds_df <- data.frame(tibble::rownames_to_column(as.data.frame(odds_ratio)))
odds_df[odds_df$rowname == "absnt>8",]$rowname <- "abseentism"
odds_df$rowname <- tolower(odds_df$rowname)
names(odds_df) <- c("predictor","odds_ratio")
