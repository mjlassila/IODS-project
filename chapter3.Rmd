...
coefficient_names <- c("gooutvlow","gooutlow","gooutmod","sex-male","absent8<")
coefficients <- round(coef(alc_glm_first)[c(1,2,3,6,10)],2)
names(coefficients)  <- coefficient_names
knitr::kable(coefficients, col.names = "Coefficient") 
```
...
odds_ratio <- exp(coef(alc_glm_first)[c(1,2,3,6,10)])
odds_df <- data.frame(tibble::rownames_to_column(as.data.frame(odds_ratio)))
odds_df[odds_df$rowname == "absnt(7,45]",]$rowname <- "abseentism"
odds_df$rowname <- tolower(odds_df$rowname)
names(odds_df) <- c("predictor","odds_ratio")
model_summary_table <- data.frame(model_summary$table[c(1,2,3,6,10),])
model_summary_table[model_summary_table$predictor == "Absnt7,45]",]$predictor <- "abseentism"
...
knitr::kable(model_summary_table %>% inner_join(odds_df))
```
