...
```{r kendall-visualization}
ggstatsplot::ggcorrmat(
  data = learning2014,             # data from which variable is to be taken
  cor.vars = age:points,
  sig.level = 0.05
)
```
...
```{r goodman-kruskal}
learning2014_grouped <- learning2014 %>%
  transmute(
    gender = gender,
    age = GroupNumeric(age,n = 6),
    attitude = GroupNumeric(attitude, n=5),
    deep = GroupNumeric(deep, n=5),
    stra = GroupNumeric(stra, n=5),
    surf = GroupNumeric(surf, n=5),
    points = GroupNumeric(points, n=10)
  )
learning_mat <- GKtauDataframe(learning2014_grouped)
plot(learning_mat, diagSize = 0.5)
```
...
learning2014_model <- lm(data=learning2014,formula = points ~ factor(gender) * attitude + deep + stra)
...
plots <- gg_diagnose(learning2014_model_simplified, plot.all = FALSE, theme = theme_bw())
