...
gg_resfitted(learning2014_model, scale.factor = 1) + theme_bw()
```


```{r qqplot, echo = FALSE}
gg_qqplot(learning2014_model, scale.factor = 1) + theme_bw()
```

```{r residual-leverage, echo = FALSE}
gg_resleverage(learning2014_model, method = "loess", se = FALSE, scale.factor = 1) + theme_bw()
```
