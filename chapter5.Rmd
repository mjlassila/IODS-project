...
```
Therefore, let's use [Goodman-Kruskal tau](https://cran.r-project.org/web/packages/GoodmanKruskal/vignettes/GoodmanKruskal.html) to explore associations between variables in the dataset. 
...
Going trough the Goodman-Kruskal tau-matrix, we can observe few interesting associations between variables. Overall, associations are not very strong, but taking into account associations which are stronger than mean value for association (0.08).

* gender and considering tea feminine
* age and considering tea feminine
...
```{r pair-comparison, cache = FALSE, echo = FALSE, fig.width=13, fig.height=9}
ggpairs(tea %>% 
...
Let's use MCA to investigate, if there are statistically observable differences between groups of people who prefer different types of tea.
...
It should be kept in mind that the proportion of green tea drinkers is very small compared to the friends of Earl Gray so that these conclusions based on biplot are not likely very strong.
...
*Scree plot* cat be used to visualize the percentage of inertia explained by MCA dimensions.
